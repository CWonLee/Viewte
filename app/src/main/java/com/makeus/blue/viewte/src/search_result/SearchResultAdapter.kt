...
import kotlinx.android.synthetic.main.item_search_result_recycler.view.*

class SearchResultAdapter(private val items: ArrayList<ResponseSearchResult>, private val context: Context): RecyclerView.Adapter<SearchResultAdapter.ViewHolder>() {
    //아이템의 갯수
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SearchResultAdapter.ViewHolder {
        val v = LayoutInflater.from(parent.context).inflate(R.layout.item_search_result_recycler, parent, false)
        return ViewHolder(v)
    }
        ...
        holder.itemView.setOnClickListener(object : OnSingleClickListener() {
            override fun onSingleClick(v: View) {
                if (items[position].getIsMemo() == 'Y') {
                    ...
                    intent.putExtra("categoriesNo", items[position].getCategoriesNo())
    ...
    inner class ViewHolder(view: View): RecyclerView.ViewHolder(view) {
        fun bindItems(data : ResponseSearchResult){
            itemView.item_search_result_tv_title.text = data.getITitle()
            ...
            itemView.item_search_result_tv_date.text = newDate
            if (data.getIsMemo() == 'N') {
                itemView.item_search_result_iv_mic.visibility = View.GONE
            }

            GlideApp.with(context).load(data.getImageUrl())
                .apply(RequestOptions.circleCropTransform())
                .into(itemView.item_search_result_iv)
        }
    }
